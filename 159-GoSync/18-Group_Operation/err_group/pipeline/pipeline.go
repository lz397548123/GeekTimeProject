...
func main() {
	ExamplegroupPipeline()
}

// ExamplegroupPipeline 一个多阶段的pipeline.使用有限的goroutine计算每个文件的md5值.
func ExamplegroupPipeline() {
	...
	for i := 0; i < numDigesters; i++ {
		g.Go(func() error {
			for path := range paths { // 遍历直到paths chan被关闭
				datam, err := os.ReadFile(path)
				if err != nil {
					return err
				}
				// 计算path的md5值，放入到c中
				select {
				case c <- result{path, md5.Sum(datam)}:
				case <-ctx.Done():
					return ctx.Err()
				}
			}
